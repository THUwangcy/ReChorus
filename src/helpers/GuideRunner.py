...
class GuideRunner(BaseRunner):
    ...
    def parse_runner_args(parser):
        parser.add_argument('--rerank', type=str, default=None,
                            help='Reranking method: boost, mmr, slack')
    ...
    def evaluate_metrics(self, dataset: BaseModel.Dataset, sort_idx: np.ndarray, topk: list, metrics: list,
        ...
        for i in range(len(dataset)):
            ...
            rec_quality.append([dataset.corpus.item2quality[candidates[idx]] for idx in sort_lst])
            ...
            for metric in metrics:
                ...
                if metric == 'HR':
                    ...
                    for set_name in eval_list:
                        ...
                        evaluations[eval_key] = ndcg_(hit, gt_rank, target_items, group).mean()
    ...
    def pred_sort(self, dataset: BaseModel.Dataset, predictions: np.ndarray):
        if dataset.phase in ['dev', 'test']:
            ...
            if self.rerank == 'slack':
                return exp_slack(dataset, predictions, max(self.topk), policy=self.exp_policy)
            if self.rerank == 'tfrom':
                return tfrom(dataset, predictions, max(self.topk), policy=self.exp_policy)
