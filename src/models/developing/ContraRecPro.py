...
class ContraRecPro(SequentialModel):
...
class ContraLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        # compute logits
        anchor_dot_contrast = torch.matmul(anchor_feature, contrast_feature.transpose(0, 1)) /self.temperature
        # for numerical stability
        logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True)
        logits = anchor_dot_contrast - logits_max.detach()  # bsz * anchor_count, bsz * n_views

        # tile mask
        mask = mask.repeat(anchor_count, contrast_count)  # bsz * anchor_count, bsz * n_views
